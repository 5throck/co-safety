# 저장탱크 운영 관리 (Tank Storage Management) Workflow

## 1. 목적
본 워크플로우는 한국 가스안전 관련 법령(고압가스 안전 관리 및 사업법 (High-Pressure Gas Safety Control and Business Act) 제13조·제15조, 산업안전보건법 (Occupational Safety and Health Act) 제44조(PSM))에 따른 저장탱크 운영 관리 업무를 관리한다.
...
## 3. 증거 기록
다중 출처의 `legal_basis`를 포함하여 `gasterm-tank-storage-record.json`을 생성한다.

## 4. 법적 근거 (Legal Basis)
출처: 본 워크플로우 `schema.yaml`의 `legal_basis`. 아래 인용 문자열은 `schema.yaml`과 정확히 일치(VERBATIM)한다.

- 고압가스 안전 관리 및 사업법 Article 13
- 고압가스 안전 관리 및 사업법 Article 15
- 산업안전보건법 Article 44 (PSM)
