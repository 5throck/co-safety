...
| `agency` / `tier` / `framework` / `jurisdiction` / `regulator` | string | Identity + authority metadata (legacy `source_mcp` fields were removed 2026-08-26; `scripts/co-safety/safety-audit.ts` v4.10.1+ validates `source_verification` on coordinate-mode files instead) |
...
| `last_updated` | date | Required — 90-day staleness reader in `scripts/co-safety/safety-audit.ts` |
...
| KR law registries remaining | 44 of 44 tracked by `scripts/co-safety/safety-audit.ts` (WARN-only progress counter) | ✅ All converted 2026-08-26 (pending OC key for k-law live pulls — Phase 4 fills `mst`) |
