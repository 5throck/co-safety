...
- [x] `emergency-response` skill frontmatter declares `legal_basis` with 3 statutory sources (OSHA-KR Art 54, SAPA Art 3, OSHA-KR Art 38) and all 4 platform copies are byte-identical (SSOT `skills/emergency-response//` + `.agents/`/`.claude/`/`.gemini/`)
