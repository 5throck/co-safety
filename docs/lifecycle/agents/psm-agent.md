...
- [x] Scope separation is tool-enforced: mi-inspection limited to PSM-covered process equipment — structural tank validation stays with gasterm's `tank-integrity-validator` skill and general aging equipment with asset-integrity-agent; boundary check implemented in `scripts/safety-audit.ts`
