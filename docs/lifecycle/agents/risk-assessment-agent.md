...
- [x] Register rollup is executable: `bun scripts/co-safety/risk-register-rollup.ts` exists with manager sign-off flags (`--manager-id`, `--signer-id`, `--signed-at`, `--dry-run`); write targets `memory/assessments/` and `memory/registers/` exist; owned skill `risk-assessment` exists at `skills/daily/risk-assessment/SKILL.md`
- [x] Role separation enforced by tooling: EHS-only scope boundary (product quality → gmp-agent/gmp-qrm; process safety → psm-agent; device risk → meddevice iso14971-risk-scorer; construction falls → ehsconst fall-hazard-assessor) is checked by `scripts/co-safety/safety-audit.ts`
