...
- [x] Register rollup is executable: `bun scripts/co-safety/risk-register-rollup.ts` exists with manager sign-off flags (`--manager-id`, `--signer-id`, `--signed-at`, `--dry-run`); write targets `memory/assessments/` and `memory/registers/` exist; owned skill `risk-assessment` exists at `skills/risk-assessment//SKILL.md`
