...
- [x] Bulk ingestion path is executable: `bun scripts/co-safety/training-ingest.ts` exists (CSV input, strict e-signature policy rejecting rows missing `signer_id`/`signed_at`); record target `memory/training/` exists and is machine-validated by `scripts/co-safety/safety-audit.ts`
