...
- Parse new MSDS documents via `skills/msds-parser//` (Mode 1 rule-based + Mode 2 ML fallback)
- Apply GHS Rev 9 classification via `skills/ghs-classifier//`
...
3. Apply GHS Rev 9 classification rules via `skills/ghs-classifier//`
4. Parse MSDS documents via `skills/msds-parser//` (Mode 1 → Mode 2 fallback)
...
| Skill | `skills/msds//{msds-parser, ghs-classifier, chemical-risk-assessment}/` |
