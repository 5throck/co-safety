...
- Facility-level living risk registers are produced by rolling up RA instances via `bun scripts/co-safety/risk-register-rollup.ts`; register rollups (`RR-*.json`) are written under `memory/registers/`.
