...
- **[2026-09-26]**: feat(config): adopt structured region profiles per workspace ADR-0091 (the co-newbiz model, fleet-wide) — `variant.json` `country_config` migrated to the structured shape (`profiles_dir`/`supported`/`default: null`, locales out; corrects the `default: "KR"` country-profiles-rule violation), `region-profiles/` added (KR.yaml provenance-validated regulatory profile — jurisdiction content shared from the co-newbiz KR baseline, `maintainer` fields localized to the co-safety roster — plus `_schema.yaml` + `_validate.ts` with identical keys/logic), and `docs/countries/KR.md` linked to the structured layer. Validator: 1 profile, 0 errors. Design doc registered: `docs/designs/2026-09-26-region-profiles-kr-adr-0091-design.md`.
...
- **[2026-09-17]**: docs(adr): backfilled decision records for eight undocumented architecture decisions (ADR-0004~0011) — CSO agent override (2026-06-04/08-28), documentation language-policy exception route (2026-06-21/08-28), evidence-runtime legal-act semantics (2026-08-24), k-law sole live statute source (2026-08-26), `scripts/co-safety/` variant script nesting (2026-08-29), skill layout flattening + `mirror: false` (2026-09-12), Universal Design Gate local activation (2026-09-12), and CI full-history requirement for the VERSION_MANIFEST gate (2026-09-16). All retroactive per the ADR-0003 pattern (`Original execution` / `Documented` dates, append-only numbering); corpus now 0000–0011, fully indexed by the regenerated skill graph (262 nodes / 295 edges). Batch design doc registered: `docs/designs/2026-09-17-adr-backfill-design.md`.

- **[2026-09-16]**: chore(ci): `safety-audit.yml` — all four `actions/checkout` steps now use `fetch-depth: 0` (full git history). The audit battery's VERSION_MANIFEST gate derives per-file last-modified dates from git log; under the default shallow checkout the regeneration fell back to checkout-time dates, making the gate's verdict environment-dependent (the ci.yml `audit` job got the same fix during the 2026-09-16 fleet resync after a permanent-drift failure). The upstream generator now tolerates shallow checkouts too (workspace PR #943); full history keeps date verification complete.
...
- **[2026-09-12]**: feat(skills)+fix(scaffolding): adopted the `accessibility-audit` skill from templates/common (WCAG 2.1 AA evaluation via axe-core — referenced by design-foundation.md §7 but absent since the 2026-08-29 skill cleanup; now the full 88-skill fleet is present), and aligned `new-domain.ts` (v1.0.2) with the flat skill layout per workspace ADR-0075 — domain scaffolding no longer creates the nested `skills/domains/<tier>/<name>/` container.

- **[2026-09-12]**: refactor(skills): flattened project skill layout to the fleet standard — 52 nested skills moved from `daily/`, `investigation/`, `emergency/`, `domains/{functional,industry}/<domain>/` to flat `skills/<name>/` (dirs renamed to frontmatter names: loto→psm-loto, moc→psm-moc, change-control→gmp-change-control, deviation-capa→gmp-deviation-capa, qrm→gmp-qrm), per workspace ADR-0075. Procedure `skill_key` values dropped layout-path prefixes (5 refs, 3 files); the 52 skills carry `mirror: false` (agent-dispatched only — sync-skills v1.6.0 opt-out); project `sync-skills.ts` updated to v1.6.0; stale `skills/_meta/SKILLS.md` removed; variant tests T-08/T-09 updated for the flat layout (test-pharma-general-profile, test-chemical-handling-profile, test-cross-domain-integration) and the safety-audit gmp-qrm role-separation path restored to a live check. Skill-graph now projects the full set (250 nodes / 294 edges); VERSION_MANIFEST regenerated (85 skills). Depends on workspace PR #891 (template flattening) — merge #891 first.

- **[2026-09-12]**: fix(skills): categorized all 85 skills via `metadata.type` frontmatter — the 52 nested skills (daily/investigation/emergency → `workflow`, domains/** → `domain`) plus `finishing-a-development-branch` (`process`) had no type, so the auto-generated `skills/SKILLS.md` index dumped them under "## Unknown". Index regenerated with proper Workflow/Domain sections; zero Unknown entries remain. Prerequisite groundwork for the skill-layout flattening migration (nested categories break upgrade-project delivery and skill-graph projection).

- **[2026-09-12]**: fix(governance): restored repo CI/hook execution — (1) GitHub Actions had been disabled at the repo level (`actions/permissions.enabled=false`), so no workflow (CI, Safety OS Audit) had run since 2026-07-11 while Dependabot entries still appeared; re-enabled with `allowed_actions=all`. (2) `.githooks/{commit-msg,pre-commit,pre-push,pre-rebase}` were committed as 100644 (only `post-checkout` was 755), so git silently skipped the local governance hooks on every commit/push since scaffold; exec bit fixed to 755. (3) `safety-audit.yml` triggers covered only the legacy `master` branch — widened to `[main, master]` to match the renamed default branch. (4) First CI run since July exposed stale workflow assumptions: `ci.yml` installed from a nonexistent root `package.json` (deps live under `scripts/`) and `safety-audit.yml`'s test-suite invoked four scripts by their pre-reorganization flat paths — both corrected (`cd scripts && bun install --frozen-lockfile`; `scripts/co-safety/test-*.ts`).
- **[2026-09-12]**: docs(architecture): refreshed `docs/_meta/architecture-overview.md` §5 audit counts to current values (workflows 191→218, evidence models 160→189, regulations 31→48, `safety-audit.ts` v4.3.0→v4.10.1; Last Updated 2026-09-12) and added SSOT pointers to the two superseded catalogs — `blueprint/04-agent-catalog.md` now points to `AGENTS.md §1` (40 live agents vs the historical 15-agent v4.0 design) and `blueprint/16-skill-catalog.md` now points to `skills/SKILLS.md` + `AGENTS.md §6` (~87 live skills). No content deleted.
- **[2026-09-12]**: feat(skills): adopted the full KR country-scoped k-* skill family (k-law, k-dart, k-kosis, k-krx, k-ecos, k-opendata) from templates/common — root cause was scaffold-era `country=none` provenance in `.claude/template-version.txt` blocking upgrade delivery (k-law was referenced by AGENTS.md and three agent files but absent on disk); corrected provenance to `country=KR`, declared manifest adoption in `variant.json` `skill_manifest.variant_specific` (prune-proof per upgrade-project v1.17.1), merged the `country-scoped:KR` env block into `.env.sample`, regenerated VERSION_MANIFEST (33 workspace skills) and skill-graph (200 nodes/229 edges, 149 ADR-0072 term nodes), added AGENTS.md §6 rows for the five new skills, refreshed `docs/country-profiles.md` to the six-skill registry state, and documented the decision in `docs/adr/0002-kr-k-skill-family-adoption.md`.

- **[2026-08-28]**: chore(hygiene): resolved the workspace language-policy follow-up via the documented `lang: ko` exception route, plus template v0.6.0 upgrade cleanup — (1) AGENTS.md/CLAUDE.md/GEMINI.md de-Koreanized: KO routing keywords, domain descriptors, and the official statute registry moved verbatim to docs/glossary/kr-safety-glossary.md (lang: ko + lang_reason: legal); (2) lang: ko + lang_reason: legal declared on the 91 flagged domain files; (3) variant-scope prune: co-game-exclusive sound-synth removed via upgrade-project v1.13.0 (variant_scoped_skills registry); (4) platform-mirror skill versions normalized to semver (108 files); (5) scripts registry: 36 missing rows added, legacy backslash paths normalized. Verified: verify-platform-lifecycle PASS, verify-scripts 103/103, validate-md-language 0 violations, validate-agents PASS.

- **[2026-08-28]**: docs(legal): verified and closed the last gaps in the `legalize_kr`/`mcp_kr_legislation` → `k-law` consolidation (2026-08-26). Audit found `README.md`, `AGENTS.md`, `agents/_shared/legal-agent.md`, and `docs/_shared/mcp-integration-guide(_ko).md` were already fully consistent, but `docs/_shared/user-guide.md`/`user-guide_ko.md` never documented the live statute-lookup capability at all — added a new "Live Statute Lookup (k-law)" section (§7, renumbering §7→8 Governance and §8→9 Legal Disclaimer in both files) covering the `LAW_API_OC` prerequisite, the owning agent (`legal-agent`), and the `k-law`/`kr_safety` MCP division of labor. Also removed 4 dead `.cache/legalize-kr` git-submodule permission entries from `.claude/settings.local.json` (gitignored, local-only file — the submodule was removed along with the old MCP servers).
- **[2026-08-28]**: chore(skills): added `api-documentation`, `documentation-writing`, `research-analysis`, `finishing-a-development-branch`, `platform-command-lifecycle-manager`, `platform-skill-lifecycle-manager` to `skills/` — generic governance/utility skills that already existed correctly in this project's `.claude/`/`.gemini/` platform mirrors but had no `skills/` SSOT entry, discovered while reconciling this project's skill set against `templates/co-safety/`. Frontmatter normalized to `scope: common`, `owner: pm`, `status: active` per this project's `skill-lifecycle-audit.ts` requirements (`scope` must be a literal `workspace|common|variant`, not a variant name). Verified: `bun scripts/skill-lifecycle-audit.ts` 67 skills scanned, 0 errors; `bun scripts/validate-skills.ts` 0 errors.
- **[2026-08-28]**: fix(agents): flattened `agents/_core/` (`pm.md`, `safety-governance-manager.md`, `safety-workflow-manager.md`) up to `agents/` — the nested `_core/` layout had produced a duplicate: `agents/pm.md` (a stale `extends`-based override stub, orphaned since the template's actual SSOT never used that path pattern) and `agents/_core/pm.md` (the real CSO runtime definition) both existed under the same registered name `pm`, causing ambiguity in `variant.json`'s `agents` array. `agents/pm.md` is now the sole file, rewritten as a proper `extends`-based override stub (extends the workspace-common `agents/pm.md`); its CSO-specific Section A/B/C content (Legal Basis / Role & Responsibilities / Operational Protocols) moved to `docs/co-safety.context.md` under a new "CSO Runtime Definition" section, aligned with the same pattern subsequently adopted by `templates/co-safety/` (see companion PR in the ai-workspace-standards repo). SGM/SWM moved with no content changes. Updated all downstream references: `AGENTS.md` (roster row + CSO content moved out to `docs/co-safety.context.md`, no longer duplicated in two places), `docs/lifecycle/agents/*.md`, `PROMOTION_CHECKLIST.md`, `docs/VERSION_MANIFEST.md`, `docs/governance/kpi-definitions.md`, `docs/_shared/domain-onboarding-guide(_ko).md`, `policies/README.md`, `variant.json` (deduplicated the `pm` entry), 3 lifecycle-script comments (`agent-lifecycle-audit.ts`, `validate-agents.ts`, `skill-lifecycle-audit.ts` — comments only, scan logic was already generic recursive). Added `version: "1.0.0"` frontmatter to SGM/SWM for schema consistency with the template. Also copied `scripts/validate-docs-links.ts` from `templates/common/scripts/` — this pre-flight gate is unconditionally invoked by `dev-sync.ts` on every `/sync` run but was never carried over during the project's L1→L3 promotion, so `/sync` has been unusable in this repo since scaffolding; discovered when running `/sync` to land this change. Registered in `scripts/SCRIPTS.md`. A second `/sync`-blocking bug surfaced right after: `.githooks/pre-commit` hardcoded the sync-context marker filename as `.sync_context.tmp`, but `dev-sync.ts` now writes a per-run-unique filename (`.sync_context.<uuid>.tmp`, exported as `DEV_SYNC_CONTEXT_FILE`) to avoid races between concurrent `/sync` runs — the hook was never updated for that rename, so every commit attempt failed context verification. Fixed the hook to read `${DEV_SYNC_CONTEXT_FILE:-.sync_context.tmp}`. Verified: `bun scripts/audit.ts` 0 errors, `bun scripts/validate-agents.ts` 80 files 0 errors, `bun scripts/agent-lifecycle-audit.ts` 39 agents healthy, `bun scripts/skill-lifecycle-audit.ts` clean (1 pre-existing unrelated warning), `bun scripts/validate-docs-links.ts` exit 0, workspace-root `bun scripts/validate-pm-extends.ts` chain valid.
- **[2026-08-27]**: feat(promotion): **Phase B promotion completed** — `templates/co-safety/` created via l3-to-variant-pipeline v1.13.1 after L0 pipeline defect fixes (user-approved option 1, 2026-08-26): **generate-variant v1.15.0** (generateAgentOverrides creates parent dirs before writeUTF8File — nested `agents/domains/**` targets crashed ENOENT since writeUTF8File does not mkdir; generateSkillDirectories + extractSkills derive skill names from the SKILL.md's immediate parent directory — the old `/skills\/([^/]+)\//` first-segment regex collapsed the 2-tier nested SSOT (`skills/<category>/<...>/<name>/`) into garbage category-named groups and an empty variant.json skill manifest despite the 4-entry `skill_manifest.variant_specific`; agents/pm.md copy now strips L0-only frontmatter `lifecycle:`/`role:` per L2 variant contract; ensureVariantInjectMarkers uses WS-09 alias regexes from context-md-schema.ts — exact `## Guidelines` match silently skipped `## Domain Guidelines`, failing the L0 VARIANT-INJECT: guidelines [REQUIRED] check; `_`-prefixed meta dirs skipped; flat layouts yield identical results, verified by test-l3-to-variant-promotion 11/11); **audit.ts 2.23.0→2.24.0** (L0+L1+L3 in lockstep: skills registry cross-check supports nested SSOT — top-level entries without a direct SKILL.md validate recursively per contained SKILL.md (immediate parent = skill dir, same discovery rule as sync-skills.ts), `_*` meta dirs skipped; resolved the 5 project FAILs `skill directory missing SKILL.md: skills\{daily,domains,emergency,investigation,_meta}\` — the ADR-0038 audit-variant.ts hook is add-only and cannot scope core checks, so the sanctioned per-file `audit_exception:` escape hatch (audit.ts checkVariantSkillSections) marks the 73 promoted skills as safety-os-skill-structure: legal_basis-gated SSOT format validated by skill-lifecycle-audit.ts/validate-skills.ts, not the generic 5-section/7-frontmatter template schema — 60 SSOT + 13 platform-only sources marked, mirrors re-synced via sync-skills.ts); **pipeline v1.13.1** (Phase 4 post-generation cleanup joins generatedVariant.variantPath — config.outputPath is undefined without `--output=`, so join(undefined) crashed Phase 4 immediately after a successful generation); **project fixes**: .gemini/settings.json `#` header comment → `"_comment"` JSON field (strict-JSON parity parse), identical `mcpServers.kr_safety` added to .claude/settings.json (parity shared-tier rule; matches generateClaudeSettings/generateGeminiSettings convention), dangling CONSTITUTION.md refs → docs/context.md in 6 carried files (agent-lifecycle-manager + script-lifecycle-manager SSOT skills, memlog commands x2, blueprint 05, code-graph v4.0 playbook), audit.ts refreshed to L1 2.24.0 copy + SCRIPTS.md/VERSION_MANIFEST rows aligned, scan-l3-project.ts @version header aligned to registry 1.3.0 (pre-existing Check A drift). **Artifacts**: templates/co-safety/ = 41 nested agent files (pm.md lifecycle-free), 73 skills x3 roots, variant.json (39-agent roster, 4 variant_specific skills, beta 0.1.0, safety type), AGENTS.md/README pair from L1 renderer, docs/co-safety.context.md with full VARIANT-INJECT marker set incl. guidelines, bilingual user-guide pair + agents/README pair authored (WS-11/Variant Contract), co-safety added to all 6 index READMEs (WS-12: EN/KO primary + templates pairs FAIL-cleared, es/ja WARN backfilled). **Condition 6**: `validate-templates --variant co-safety` exit 0, 0 errors, P-01 parity PASS, Variant Contract 11/11, non-vacuous (73 skills checked). **Verification**: pipeline 6/6 phases (parity incl.); project `bun scripts/audit.ts` exit 0 / 0 FAIL and `bun scripts/safety-audit.ts` 1077 files 0 errors; workspace `validate-templates --variant co-safety` exit 0; workspace-root audit.ts reduced 158→1 substantive failure class. **Known follow-up (non-gating)**: workspace-root language validation reports 182 Korean-content violations in templates/co-safety (84 docs/workflows, 59 skills, 37 agents, AGENTS.md+README) — Korean dispatch triggers, statutory citations, and bilingual workflow docs are intrinsic to this Korea-only variant; requires a workspace language-policy decision (per-variant ko-allowlist vs `lang: ko` legal/source-material exceptions vs backtick proper-noun wrapping), not mechanically resolvable per-file without that call.
- **[2026-08-26]**: chore(mcp)+feat(promotion): remove legalize_kr/mcp_kr_legislation MCP servers (k-law sole content source; kr_safety retained — configs x4, mcp/ dirs, start-mcp/test-runner/safety-audit 4.10.1/migrate 1.0.2 updated, 51 regulation yamls source_mcp→source_verification/manual-curation, caches .cache/{legalize-kr,admrule-kr} purged, docs/guides/workflow-READMEs swept incl. blueprint 16/17/App-C legalize-kr-sync rows marked removed, GITHUB_TOKEN orphaned→env.sample pruned, VERSION_MANIFEST regenerated 40 agents/78 skills/61 scripts); **Phase B promotion attempted** (l3-to-variant-pipeline v1.13.0): pre-flight gates cleared after in-project fixes — core scripts refreshed to immutable L1 copies (audit.ts 2.6.5→2.23.0, dev-sync.ts 1.5.0→1.7.6, +helpers/{pm-md-parser,security-validator,context-sections}.ts, lib/{language-guard,encoding-utils}.ts; variant logic migrated to new scripts/audit-variant.ts 1.0.0 per ADR-0038 pluggable hook: CSO safety-audit gate + 5 domain test suites), agents/pm.md scaffolded as additive CSO override (extends chain, VARIANT-SECTION blocks), AGENTS.md restructured to L1 §-scheme with 6 VARIANT-* injection anchors, coke-oven SKILL.md homoglyph μ→u, test-runtime-tools T-09e expectation aligned to documented default ('planned'); Phases 1→3.7 all pass (core-script integrity, pm.md diagnosis, WS-09 context, AGENTS.md markers) but **Phase 4 BLOCKED**: generate-variant.ts crashes ENOENT on nested agents (writeUTF8File no mkdir for agents/domains/... targets) and its skill grouping regex /skills\/([^/]+)\// mangles the 2-tier skills/ SSOT into garbage skills/{daily,domains,...}/SKILL.md entries — nested agents/skills architecture (deliberate, tested by T-08, validate-agents v1.1.0 recursive scan) conflicts with the pipeline's flat-roster assumption (all 11 sibling L3 projects flat); partial templates/co-safety/ artifact removed; **Condition 6 platform parity: NOT exercisable** (validate-templates --variant co-safety = vacuous 0-variant pass); **Condition 7 user-confirmed 2026-08-26**. Verification: safety-audit.ts 1077 files 0 errors; audit.ts all-pass under pre-refresh core (222 baseline); post-refresh core audit surfaces 5 nested-skills-dir FAILs (known conformance debt, blocked pending flatten/architecture decision); domain tests 90/90, runtime-tools 20/20, pharma 10/10, chemical 12/12.
- **[2026-08-26]**: fix(compliance): close the 2026-08-26 project-review findings (PM-orchestrated, 13-batch specialist remediation) — **legal_basis gate extension**: safety-audit.ts v4.9.1 adds SKILL.md frontmatter scanning for all 62 skills (tolerant of top-level and metadata-nested arrays, `type: process` platform-tooling exemption) plus variant.json `skill_manifest` validation (array >= 3, specific-article regex accepting named 고시/「」instruments); sync-skills.ts v1.4.0 adds sha256 SSOT-vs-platform `[DRIFT]` reporting before copies. **manifest**: variant.json 4 skill_manifest legal_basis entries converted from single-article strings to verified >= 3-article arrays (sources: validated skill frontmatters, compliance-agent Section A, SAPA.yaml). **skills**: psm-moc SSOT restored to superset (RIC bypass criteria + temporary-change expiration rules, byte-parity with platform copy); permit-to-work gains SAPA 제4조 third citation (Art 98-103 confirmed NOT permit-to-work per glossary); ~50 domain/EHS skills received or restored structured frontmatter legal_basis derived from owner agent Section A / sibling validated skills / regulations yaml canon ([UNVERIFIED]-flagged where canon lacks corroboration); glp-study-protocol-validator trigger 보존기간 10년→3년 per OSHA-KR 제57조 canon. **emergency cluster**: 중대재해 definition corrected to statutory SAPA 제2조 language across emergency-agent.md, emergency-response skill AND the root canon regulations/KR/SAPA.yaml Article 2 summary; response status enum extended (+contained/resolved) so the RCA-handoff state is recordable; required severity field aligned to fire-response schema enums; sample record raised to 3 legal sources; workflow_id wired into all 8 emergency evidence schemas ($ref common.schema.json definitions); ess-fire-risk-assessor escalation now routes to emergency-agent/emergency-response workflow; gas-dispersion-analyzer names concrete consumers. **PSM/domains**: process-hazard-screening applicability re-based on 시행령 제43조 별표 13 규정량 (US OSHA 10,000 lb demoted to comparative footnote); LOTO delegation clauses to psm-loto added in food/waste/steelmaking agents (shipbuilding pattern); psm-agent dispatch triggers gain LOTO/Lockout/Tagout + KPIs enriched from scope.md §7; steelmaking HPGSCA mis-citation Art 17→13 (시설·용기의 안전유지) per High-Pressure-Gas-Safety.yaml + 2026-08-07 remediation canon; scope.md Element 13 renumbered (was duplicate Element 4) + 12-vs-14 element reconciliation note; gmp change-control/deviation-capa minimum sources 2→3 (DEFAULT_MIN_LEGAL_BASIS) and stale single-source PSM-MOC claim corrected. **governance**: pm.md CSO Section A rebuilt to 5 article-specific entries from POL-001 §3; POL-001 enumerates defense-aerospace.yaml (26/26 match), v1.1.0 revision-history row appended, lang: ko frontmatter added; policies/README umbrella-naming note + v1.1.0 sync; blueprint governance doc reworded to layered language policy (replacing blanket English rule) + KPI catalog refreshed to 6 KPIs + SAPA metrics; SGM tools table gains Grep/Bash rows; _core 3-file stale last_updated bumped. **training/evidence**: training-record.json first_aid anchor de-Article-13'd per 2026-08-24 arbitration; e_signature/nomenclature/audit_trail blocks added to compliance/plan/curriculum schemas; tbm-record.json gains attendee_refs + makeup_training_ref; contractor reverse-intake protocol in training-agent.md Section C + contractor_flag/company_name fields; by_training_type enum completed to 7 types. **audit trail enforcement**: finding/corrective-action base schemas v3.0.0 make audit_trail REQUIRED with 18 legacy FIND/CA records honestly backfilled (created_at/by derived verbatim from file-creation git commits, migration provenance block, evidence-models/migrations/README.md record appended); CA-2026-0009 legal_basis inherits linked FIND per audit-agent.md:82 rule; MEMORY.md archive links fixed + dev-sync residue removed; PROMOTION_CHECKLIST paths corrected to Projects/co-safety with honest pending annotation for unimplemented l2-to-variant-pipeline.ts; session log deleted-vs-modified wording corrected. **risk integration**: iso14971-risk-scorer gains band-exact Cross-Walk appendix to EHS 1-25 scale (consult-mode); risk-assessment-agent.md Section C outbound/inbound dispatch protocols added (psm PHA / meddevice / gmp QRM / msds); canonical control_status 6-value lifecycle set unified across risk-assessment-record v3.1.0 + risk-register-record v1.2.0; risk-register-rollup default for new entries changed in_progress→planned (never overstate progress); chemical-risk-assessment writes through to risk-register records (Art 36 completeness, planned init) + 4-entry legal_basis; meddevice/pharma aliases marked deprecated. **AGENTS.md**: dead pm.md link → agents/_core/pm.md; ghs-classifier roster citation Article 243→104. Verification: `bun scripts/safety-audit.ts` 1078 files 0 errors 0 warnings; `bun scripts/audit.ts` 222 PASS / 0 FAIL / 0 WARN; sync-skills re-run 0 DRIFT.
- **[2026-08-26]**: chore(cleanup): L3→variant promotion pre-flight cleanup — **legacy script removal**: deleted 15 legacy `.ps1`/`.sh`/`.sed` scripts (audit, setup, install-bun, upgrade-project, sync-md, gen-pr-body, cleanup-completed-md pairs + fix-parse-agent.sed) per ADR-0036 TS-only policy; updated 4 stale references (GEMINI.md audit hint, qa-gate.ts error message, gen-pr-body.ts PR template, verify-memory.ts hint); SCRIPTS.md registry rows removed and Tier-1 section rewritten; README/README_ko rewritten TS-only. **variant.json normalization**: `type: security` → `variantType: safety` (matches new workspace registry type), `inherits_common: "1.0.0"` → `"templates/common"`. **WS-09 restructure**: `docs/co-safety.context.md` rebuilt with required slot order (Tech Stack → Agents → Skills → Development Workflow → Domain Guidelines → File Organization Policy → Domain Rules), verified via shared `context-md-schema.ts` checker — passes pipeline PHASE 3.6 blocking gate. **CONSTITUTION.md removal**: deleted governance-index file (content already lives in CLAUDE.md/GEMINI.md per template standard); `audit.ts` relaxed — existence check no longer FAILs when absent (L3 variants omit it), §5.2 root-file allow-list updated, memory-check gate simplified, IS_WORKSPACE_ROOT detection now variant.json-first; remaining scripts (dev-sync, agent-verify, lifecycle audits, verify-scripts, gen-pr-body) already had existsSync fallbacks. Verification: `bun scripts/audit.ts` exit 0, all checks passed.
- **[2026-08-24]**: fix(regulations): resolve the image-only annex blockers via live law.go.kr attachment acquisition - **KP-GMP**: 별표 1 「의약품 제조 및 품질관리기준」 PDFs downloaded + text-extracted (flSeq 162113631 current / 149321933 prior, 31pp each); full 호 structure verified (제1호~제15호 incl. 제12호 변경관리, 제13호 자율점검); MFDS-GMP.yaml key_articles re-keyed from internal taxonomy to statutory anchors (VERIFIED status). **LPG/Hydrogen annexes**: 시행규칙 별표 4 (25pp), 별표 5 (17pp), 수소 별표 5 (4pp) acquired via flSeq mapping; storage-distance tables verified (LPG 용기충전 24-39m tiers 지하x0.7, 자동차충전소 17-30m/12-20m, 저장설비 17-30m; 수소 보호시설 17-30m/12-20m, 화기 8m, 산소 5m; 특례고시 figures reconciled as derived minimum rows); legacy "15-50m school/hospital" claim confirmed absent from current law - markers -> VERIFIED with source URLs. **City-Gas pre-emptive revalidation** (next crossing was 2026-10-03): sole indexed article verified; material drift fixed in kgs_code_delegation (standard-setter is 가스기술기준위원회 with MOTIE approval, KGS hosts secretariat only - misattribution mattered for a legal-basis-gate platform); date bumped -> next crossing 2026-11-22. **GMP instrument citation cluster closed repo-wide** (~60 files): 약사법 manufacturing anchor Article 34->37 swept across agents/profiles/schemas/skills/evidence-models/docs (26 sites; Art 34 correctly retained only where clinical-trial approval context; bonus: 7 GLP evidence models corrected 34->34-3 비임상시험기관 지정); 11 GMP evidence-model enums re-keyed to 별표 1 호 scheme; legal-glossary v1.0.8 adds 의약품등의안전에관한규칙 entries and retires 의약품등기준규정 as alias; phantom instrument "KP-GMP 시행규칙" (qrm/change-control/deviation-capa skills) verified nonexistent and replaced with 약사법 Art 37; test-pharma-general-profile.ts assertion updated 34->37. Verification: safety-audit 1024 files 0 errors; scenarios 90/90; runtime-tools 20/20; pharma-profile 10/10.
...
- **[2026-08-28]**: `docs/co-safety.context.md` — "27 industry/functional domains" → "30 (8 functional + 22 industry)"; `industry-profiles/` count 14 → 26 (session-loaded context doc).
- **[2026-08-28]**: `docs/_shared/domain-classification-guide{,_ko}.md` — "15 active domains (5 functional + 10 industry)" → "30 (8 functional + 22 industry)"; §2 Tier-1 functional table: added `risk-assessment`, `incident-investigation`, `asset-integrity`.
- **[2026-08-28]**: `docs/_shared/domain-onboarding-guide{,_ko}.md` — §5 Active Domains Registry: added the 15 missing domains (12 industry + 3 functional); `ehssemi` → `semicon`.
- **[2026-08-28]**: `docs/_shared/user-scenarios{,_ko}.md` — nav "12-domain" → "30-domain"; EN-only: renumbered the duplicate "Scenario 6" → "Scenario 7" (ko already correct).
- **[2026-08-28]**: `docs/user-guide/domain-quick-reference_ko.md` — removed **deleted HPGSCA Art-14** citations (gasterm, semicon); replaced with verified in-force articles via `legalize_kr` (gasterm Art 13/15; semicon Art 11/13/15/24/26) and corrected the wrong statute `CCA` → `CCA (CCA)` for semicon (matches its schemas; CCA Art 20 is the actual citation).

**P1 — design docs:**
- **[2026-08-28]**: `docs/_meta/architecture-overview.md` §3 — updated Workflows/Evidence-Models counts for all 12 promoted industries to match the maturity matrix (most 2–3 → 5). Header/diagram (30) untouched.
- **[2026-08-28]**: `docs/_meta/ROADMAP.md` — §2.1 "Newer 12" cohort averages updated to post-maturation values; Tier-2 target marked ✅ Achieved 2026-08-07 (ahead of 2026-10-31); Tier-2 skill criterion corrected (3 → ≥1).

**P2 — polish:**
- **[2026-08-28]**: `docs/user-guide/field-ehs-operational-guide_ko.md` — heading "newly expanded 7 major industries" → "12 major industries"; typo "railway/transportation" → "railway/transportation".
- **[2026-08-28]**: `docs/_shared/tutorial{,_ko}.md` — Last Updated date bumped to 2026-08-07.
- **[2026-08-28]**: `docs/domains/functional/psm/scope.md` + `domain-classification-guide{,_ko}.md` §4 — last `ehssemi` residuals → `semicon` (PSM-integration "planned/future" status retained).
...
- **[2026-08-28]**: **evidence-models (14 JSON)** — `legal_basis` enum/example citation values unified to the long form, aligned to the #94-remediated workflow schemas. `completion-inspection` evidence-model aligned to its schema (Art 28→13, Art 17→13). `description` prose left as-is (short form is natural Korean).
- **[2026-08-28]**: **Deleted-Art-14 accuracy catch (12 citations, 9 files)** — profiles (logistics-port, defense-aerospace ×2, semicon-cleanroom, semiconductor), agents (defense, logistics, semicon, steelmaking), and scope.md (steelmaking, semicon, logistics, defense) still cited **deleted Art 14**; all replaced with verified **Art 13 (upkeep of facilities and containers)**.
- **[2026-08-28]**: **regulations/KR/High-Pressure-Gas-Safety.yaml** — formal citation strings → long form (YAML identifier keys and legalize_kr lookup fields left short form).
- **[2026-08-28]**: **industry-profiles (5)** + **agents (4)** + **docs/domains/industry/scope.md (4)** — formal citation lists with article numbers → long form; statute-name-only prose mentions left as-is.
- **[2026-08-28]**: **Left unchanged (correct)**: `legal-glossary.yaml` dict key, `industry-regulatory-anchors.yaml` lookup keys, casual prose, and the 3 historical archival docs (`docs/_meta/archive/`, `docs/superpowers/{plans,specs}/`) — not edited to avoid falsifying history.
...
- **[2026-08-28]**: **Tier 1 — deleted Art 14 (3 SKILLs):** `tool-box-meeting` (semicon row: dropped deleted Art 14, kept Art 17 — topically correct for gas cylinders/container), `painting-coating-fire-toxic-planner` (Art 14/28 → workflow's verified Art 11/13/15/24/26 set), `dangerous-cargo-handling-planner` (Art 14 → Art 13 facility safety + long statute name).
- **[2026-08-28]**: **Tier 2 — in-force Art 17/28 topic-label/mismatch (4 SKILLs):** `tool-box-meeting` (gasterm row Art 17 → Art 13 — gas-pipe opening is facility, not container inspection), `coke-oven-pah-heat-stress-planner` (Art 17 mislabeled "gas facility" → Art 13 upkeep of facilities and containers), `pyrophoric-gas-emergency-responder` (**Art 28 mislabeled "high-pressure gas emergency response" → Art 26 accident reporting** — Art 28 is KGS establishment, not emergency response; fixed across all 7 occurrences + cleared stale UNVERIFIED disclaimers), `completion-inspection` SKILL (Art 28 "completion inspection" mislabel → Art 13, aligned to the #94-remediated workflow schema).
- **[2026-08-28]**: **Tier 3 — statute-name short→long form (gasterm construction family):** `construction-permit-overview`, `pre-construction-technical-review`, `mid-construction-inspection` — SKILL + workflow schema/README (`HPGSCA Article 22-2` → `HPGSCA Article 22-2`; Art 22-2 in-force, correct — name consistency only).
- **[2026-08-28]**: **Propagation:** all 9 source SKILLs mirrored to `.claude/skills/`, `.gemini/skills/`, `.agents/skills/` via `sync-skills.ts`.
...
- **[2026-08-28]**: **`powergen/boiler-steam-system-safety` README §1** — removed HPGSCA Art 17 (container inspection) as a **topicality mismatch**: the compliance-agent verified via `legalize_kr.parse_law_structure` that HPGSCA scope (Art 1/3) is high-pressure *gas* and its containers, refrigerators, specific equipment; a power-plant steam boiler produces high-pressure *steam (vapor)* — not stored gas — and its drum is not a HPGSCA "containers", so neither Art 17 nor Art 13 applies. README §1 now aligns VERBATIM with the schema `legal_basis` (Electric Utility Act Article 47, power-generation facility safety regulation, OSHA-KR Article 44 (PSM)) — also correcting OSHA-KR Art 98→Art 44(PSM) and adding the previously-missing power-generation facility safety regulation. Schema untouched (no HPGSCA). 3 sources retained (≥3 ✓).
- **[2026-08-28]**: **Findings:** `memory/findings/compliance-2026-08-07-codebase-hpgsca-remediation.md` §10.5. Audit: 872 files, 0 errors.
...
- **[2026-08-28]**: **15 workflow schemas** across 5 domains (gasterm 10, asset-integrity 2, defense 1, logistics 1, steelmaking 1) — stale `HPGSCA Article 14/17/28` → verified in-force articles. Art 14 (deleted 1999.2.8) replaced unconditionally; Art 17 (container inspection) / Art 28 (establishment of KGS) evaluated per-workflow and replaced where topic-mismatched; statute-name unified to the long form `HPGSCA`. ≥3 `legal_basis` preserved in all 15.
- **[2026-08-28]**: **9 gasterm READMEs** — §6 Legal Basis synced VERBATIM with each schema; §7 HPGSCA `legalize_kr` verification note added.
- **[2026-08-28]**: **`powergen/fuel-handling-safety` README** §1 — deleted Art 14 → Art 13 (the 16th deleted-Art-14 case); statute-name unified to long form. Schema has no HPGSCA citation (prose-only fix).
- **[2026-08-28]**: **4 shipbuilding READMEs** (welding + painting, Ko + En) §4 + §7 — corrected wrong HPGSCA topic-label prose (Art 11↔15 swapped; Art 13/24/26 misattributed) to the verified `legal-glossary.yaml` topics. Article citation NUMBERS were already correct; only the descriptive LABELS were wrong. semicon + steelmaking READMEs verified clean (no change).
- **[2026-08-28]**: **`regulations/KR/legal-glossary.yaml`** + anchor YAML — no change needed (already canonical from #93).
...
- **[2026-08-28]**: **4 semicon workflow schemas** (`silane-gas-leak-response`, `special-gas-handling`, `semicon-scrubber-maintenance`, `tbm-pre-work-briefing`) — stale `HPGSCA Article 14/17/28` → verified **Article 11/13/15/24/26** (matching the shipbuilding/steelmaking 5-article schema convention); `[UNVERIFIED]` markers removed; statute-name unified to the long form. Verified live this session via `legalize_kr.parse_law_structure` (MST 283919): Art 14 = deleted 1999.2.8, Art 11/13/15/24/26 = substantive in-force.
- **[2026-08-28]**: **2 silane READMEs** (Ko + En) — §6 Legal Basis remediated (VERBATIM with schema); §7 HPGSCA verification note added (mirrors shipbuilding framing; documents that the `kr_safety` catalog is stale for HPGSCA); §8 "Unverified Citations" → "Verification History".
- **[2026-08-28]**: **`regulations/KR/industry-regulatory-anchors.yaml`** — semicon HPGSCA `adjacent_laws` block remediated (Art 14/17/28 → 11/13/15/24/26, `substantive: true`); semicon `verification` block cleared; gaps HPGSCA entry updated ("RESOLVED for shipbuilding/steelmaking/semicon").
- **[2026-08-28]**: **`regulations/KR/legal-glossary.yaml`** (v1.0.5 → v1.0.6) — HPGSCA `regulator` corrected `KGS / MOIS` → `MOTIE (MOTIE) / KGS (KGS)`. Article list (Art 11/13/15/22-2/24/26) unchanged.
- **[2026-08-28]**: **Known residual (separate follow-up):** stale HPGSCA Art 14/17/28 citations remain in 15 workflows across gasterm (10), defense, logistics, steelmaking, and the asset-integrity functional domain. Art 14 (deleted) cases are unambiguous bugs; Art 17/28 are in-force articles requiring per-workflow topic judgment (e.g., gasterm tank-inspection citing Art 17 may be correct). Scoped out for a dedicated codebase-wide HPGSCA compliance pass.
...
- **[2026-08-28]**: **5 industry-unique workflows** (`workflows/domains/industry/`) — law-first, compliance-verified via live MCP (kr_safety + legalize_kr), `status: active`:
  - **[2026-08-28]**: `datacenter/rack-cabling-fall-protection` (OSHA-KR Art 36/57, ESCA Art 16/22, SAPA Art 4–7)
  - **[2026-08-28]**: `food/thermal-hazard-control` (BFS Art 16, FSA Art 48, OSHA-KR Art 36, SAPA Art 4)
  - **[2026-08-28]**: `semicon/silane-gas-leak-response` (HPGSCA Art 14/17/28, DSSMA Art 27, CCA Art 24, SAPA Art 4)
  - **[2026-08-28]**: `cosmetics/solvent-exposure-control` and `cosmetics/powder-dust-control` (OSHA-KR/MSDS Art 110, CA Art 5, K-REACH Art 10, SAPA Art 4)
- **[2026-08-28]**: **4 industry Skills** (`skills/domains/industry/`, propagated to `.claude/`/`.gemini/`/`.agents/` via sync-skills): `rack-fall-protection-planner`, `thermal-burn-prevention-planner`, `pyrophoric-gas-emergency-responder`, `cosmetics-solvent-exposure-monitor`. skill-lifecycle-audit: 53/53 healthy.
- **[2026-08-28]**: **3 evidence models** (`evidence-models/domains/industry/`, v1.0.0, `status: active`): `datacenter-clean-agent-suppression-record`, `food-foreign-material-detection-record`, `semicon-waste-neutralization-record`. Total EM count 162 → 165.
- **[2026-08-28]**: **`workflows/_shared/REFERENCE-APPLICATION-GUIDE.md`** — practitioner guide for adopting shared workflows via the `references:` pattern (meeting Action Item A-08).
- **[2026-08-28]**: **`memory/findings/compliance-2026-08-07-phase1-group-a.md`** — MCP-verified `legal_basis` sign-off report (per-workflow VERIFIED/DISCREPANCY/UNVERIFIABLE classification with evidence; 3 supplementary gaps flagged, non-blocking).

### Changed

- **[2026-08-28]**: **`docs/_meta/domain-maturity-matrix.md`** — cosmetics, datacenter, food, semicon promoted Tier 1 → Tier 2 (Operational). Industry distribution 9/1/12/0 → 9/5/8/0; Tier-1 maturation target list 12 → 8.
- **[2026-08-28]**: **`scripts/safety-audit.ts`** (v4.3.0 → v4.3.1) — effective `legal_basis` resolution per REFERENCE-SPEC §4: when a workflow schema has a `references:` block, the audit now follows it to the shared base and applies `overrides.legal_basis.add`/`replace` for the ≥3 check. Resolves the `cosmetics/tbm` thin-reference (effective basis 10 entries). Legacy top-level `legal_basis` fast path unchanged; 0 regressions across 815 files.

### Fixed

- **[2026-08-28]**: **`scripts/scaffold-industry.ts`** (v0.1.0 → v0.1.1) — `evidence_model` field now emits a bare filename (matching `scripts/new-domain.ts` and the 160+ mature workflows) instead of a wrong-depth relative path (`../../../../…`); the 5 generated industry schemas aligned. Latent generator bug that would otherwise replicate across the 8 remaining Phase 2–3 industries.
...
- **[2026-08-28]**: **8 industry-unique workflows** (`workflows/domains/industry/`, `status: active`) — law-first, compliance-verified via live MCP (kr_safety + legalize_kr); all `draft`→`active` after the ≥3 VERIFIED `legal_basis` floor was met:
  - **[2026-08-28]**: `battery/{battery-cell-formation-electrical-safety, battery-cathode-powder-dust-control}` (ESCA Art 16/22, DSSMA Art 5/15/27, OSHA-KR Art 36/57, SAPA Art 4–7)
  - **[2026-08-28]**: `biotech/{biotech-bsl-lab-aerosol-control, biotech-biological-spill-response}` (BSA Art 13, LMO-Act Art 22/24, OSHA-KR Art 36/57, SAPA Art 4–7)
  - **[2026-08-28]**: `defense/{defense-munitions-storage-magazine-safety, defense-weapons-assembly-composite-solvent}` (FSESA Art 9/23, DAA Art 53, OSHA-KR Art 36/57, SAPA Art 4–7)
  - **[2026-08-28]**: `logistics/{logistics-dangerous-cargo-handling, logistics-forklift-pedestrian-strike-prevention}` (PSSA Art 6/8/9, DSSMA Art 20, OSHA-KR Art 36/57/100, SAPA Art 4–7)
- **[2026-08-28]**: **4 industry Skills** (`skills/domains/industry/`, propagated to `.claude/`/`.gemini/`/`.agents/` via sync-skills): `hv-cell-formation-electrical-safety-planner` (battery), `bsl-lab-aerosol-control-planner` (biotech), `munitions-magazine-storage-safety-planner` (defense), `dangerous-cargo-handling-planner` (logistics). Each grounded in anchor statutes previously uncited by existing workflows. skill-lifecycle-audit: 57/57 healthy (was 53).
- **[2026-08-28]**: **4 evidence models** (`evidence-models/domains/industry/`, v1.0.0, `status: active`): `battery-electrolyte-nmp-exposure-record`, `biotech-bsc-certification-record`, `defense-test-range-blastoverpressure-record`, `logistics-yard-rack-collapse-prevention-record`. Total EM count 173 → 177.
- **[2026-08-28]**: **`regulations/KR/Port-Safety-Special-Act.yaml`** — registered PSSA (PSSA, MST 283835, lawIdCode 014131, MOF/MOF); 12 articles verified via legalize_kr. Resolves a phantom-statute citation in the logistics workflows.
- **[2026-08-28]**: **`memory/findings/compliance-2026-08-07-phase2-group-b.md`** — MCP-verified `legal_basis` sign-off report (per-workflow VERIFIED/DISCREPANCY/UNVERIFIABLE classification; 8/8 workflows meet the ≥3 VERIFIED floor).

### Changed

- **[2026-08-28]**: **`docs/_meta/domain-maturity-matrix.md`** — battery, biotech, defense, logistics promoted Tier 1 → Tier 2 (Operational). Industry Tier-1 target list 8 → 4. Summary Statistics reconciled to row-accurate counts: 9 Tier 3 / 11 Tier 2 / 8 Tier 1 / 2 Tier 0 (total 30); industry distribution 8 / 10 / 4 / 0.

### Fixed

- **[2026-08-28]**: **`regulations/KR/industry-regulatory-anchors.yaml`** — PSSA article topic corrections: Art 4 (procedural "relationship to other laws"), Art 5 (port-participant basic duties), Art 6 (port-participant safety obligations — substantive safety-obligation article), Art 8 (safety training), Art 9 (self-safety-management plan). Added `substantive: true/false` flags for machine-readable downstream use. Pre-existing Phase 0 Task A-03 error surfaced by live MCP verification.
- **[2026-08-28]**: **`regulations/KR/legal-glossary.yaml`** (v1.0.4 → v1.0.5) — PSSA entry enhanced with statute_file pointer and Art 4-vs-6 substantive/procedural clarification; expanded article coverage.
- **[2026-08-28]**: **LMO-Act Art 22/24** upgraded UNVERIFIED → VERIFIED (confirmed via `kr_safety.search_osha_regulations`) — strengthens the biotech legal_basis.
...
- **[2026-08-28]**: **8 industry-unique workflows** (`workflows/domains/industry/`, `status: active`) — law-first, compliance-verified via live MCP (kr_safety + legalize_kr); all `draft`→`active` after the ≥3 VERIFIED `legal_basis` floor was met:
  - **[2026-08-28]**: `railway/{railway-rolling-stock-maintenance-loto, railway-bridge-viaduct-fall-prevention}` (RSA Art 45/48, ESCA Art 16/22, OSHA-KR Art 36/57, SAPA Art 4–7)
  - **[2026-08-28]**: `shipbuilding/{shipbuilding-painting-coating-fire-toxic, shipbuilding-welding-fume-gas-safety}` (HPGSCA Art 11/13/15/24/26 — remediated, OSHA-KR Art 36/57, SAPA Art 4–7)
  - **[2026-08-28]**: `steelmaking/{steelmaking-coke-oven-pah-heat-stress, steelmaking-hot-rolling-mill-crush-burn}` (HPGSCA Art 11/13/15/24/26 — remediated, OSHA-KR Art 36/57, SAPA Art 4–7)
  - **[2026-08-28]**: `waste/{waste-designated-hazardous-chemical-treatment, waste-landfill-methane-anaerobic-explosion}` (WCA Art 13/25, CCA Art 23, BFS Art 16, OSHA-KR Art 36/57, SAPA Art 4–7)
- **[2026-08-28]**: **4 industry Skills** (`skills/domains/industry/`, propagated to `.claude/`/`.gemini/`/`.agents/` via sync-skills): `rolling-stock-maintenance-loto-planner` (railway), `painting-coating-fire-toxic-planner` (shipbuilding), `coke-oven-pah-heat-stress-planner` (steelmaking), `landfill-methane-anaerobic-explosion-planner` (waste). Each grounded in industry signature hazards. skill-lifecycle-audit: 61/61 healthy (was 57).
- **[2026-08-28]**: **4 genuine evidence models** (`evidence-models/domains/industry/`, v1.0.0, `status: active`) authored with full field schemas: `railway-thermite-welding-hot-work-record` (RLW-TW), `shipbuilding-pre-hot-work-gas-free-record` (SHIP-GF), `steelmaking-continuous-casting-cooling-water-record` (STEEL-CC), `waste-designated-waste-manifest-record` (WST-MAN). Plus **8 scaffold-generated EM skeletons** (double-prefixed, `status: draft` pending specialist fields). Total EM count 177 → 189.
- **[2026-08-28]**: **`memory/findings/`** — `phase2-group-c-wf-review.md` (SWM workflow selection + 14 rejections), `compliance-2026-08-07-phase2-group-c-anchors.md` (anchor verification + HPGSCA remediation), `compliance-2026-08-07-phase2-group-c-wf.md` (per-WF activation sign-off).

### Changed

- **[2026-08-28]**: **`docs/_meta/domain-maturity-matrix.md`** — railway, shipbuilding, steelmaking, waste promoted Tier 1 → Tier 2 (Operational). Summary Statistics reconciled: 9 Tier 3 / 11→**15** Tier 2 / 8→**4** Tier 1 / 2 Tier 0 (total 30); industry distribution 8 / 10→**14** / 4→**0** / 0. Phase 2 industry target list 4 → **0 (complete)**.

### Fixed

- **[2026-08-28]**: **HPGSCA (HPGSCA) deleted/mismatched article remediation** — `regulations/KR/industry-regulatory-anchors.yaml` (shipbuilding & steelmaking `adjacent_laws`), `regulations/KR/High-Pressure-Gas-Safety.yaml`, and `regulations/KR/legal-glossary.yaml`: Art 14 (deleted 1999.2.8 — deleted) and Art 17/28 (topic mismatches) replaced with verified Art 11/13/15/22-2/24/26 via `legalize_kr` (authoritative law.go.kr full-text). Critical because `scaffold-industry.ts` auto-fills `legal_basis` from the anchor table — the phantom Art 14 would otherwise have propagated into all 4 shipbuilding/steelmaking workflows. Note: the `kr_safety` MCP catalog is stale for HPGSCA (still indexes deleted Art 14); prefer `legalize_kr` for HPGSCA verification.
- **[2026-08-28]**: **Waste CCA statute-name correction** — `workflows/domains/industry/waste/{waste-designated-hazardous-chemical-treatment, waste-landfill-methane-anaerobic-explosion}/schema.yaml`: "K-REACH (ARECA) Article 23" (ARECA / K-REACH name) → "CCA Article 23" (CCA). accident-preparedness substance (accident-preparedness substance) management is CCA Art 23, not ARECA; ARECA has no Art 23 on this topic. Verified via `regulations/KR/CCA-Chemical-Control.yaml`.
- **[2026-08-28]**: **`scripts/audit.ts`** (v2.6.4 → v2.6.5) — added `railway`, `shipbuilding`, `steelmaking`, `waste` to the `knownCategoryDirs` skill allow-list. Without this, the audit flagged the 4 new industry skill folders as "missing SKILL.md" (any non-listed dir under `skills/domains/industry/` is treated as a skill leaf that must contain SKILL.md). Latent registration step missed when the Group C skills were created; mirrors the Group A/B industry registration.

### Changed (2026-08-06 — Multi-Platform Governance: 4-Platform Explicit Support)

- **[2026-08-28]**: **CONSTITUTION.md**: Rewrote from Claude-centric 2-platform index to platform-neutral 4-platform governance index. Now explicitly documents all four supported client surfaces (Claude Desktop App, Claude Code → CLAUDE.md; Antigravity, Antigravity CLI → GEMINI.md), platform skill layers (`skills/` SSOT → `.claude/`/`.gemini/`/`.agents/` mirrors), and a platform feature difference summary.
- **[2026-08-28]**: **CLAUDE.md + GEMINI.md §10 (Platform Parity)**: Added `.agents/skills/*/SKILL.md` row to the lifecycle management table — documents that `.agents/` is the Antigravity shortcut-skill layer auto-mirrored by `bun scripts/sync-skills.ts` (do NOT hand-edit; `.agents/skills.json` regenerated by same script).
- **[2026-08-28]**: **CLAUDE.md + GEMINI.md §10.5 (Platform Feature Matrix)**: New section documenting per-platform capability differences across 4 surfaces (PostToolUse hooks, subagent dispatch, slash commands, skill discovery, `.agents/` layer, MCP, background tasks, git auto-commit). Includes manual alternatives for unavailable features (e.g., Antigravity lacks PostToolUse hooks → manually run `bun scripts/audit.ts`).
